...
public:
    ...
    const
    {
        ...
        const double NdotRayDirection = dot(N, r.direction());
        if (std::abs(NdotRayDirection) < std::numeric_limits<double>::epsilon()) { // Almost 0
            return -1.0; // They are parallel, so they don't intersect!
        }

        // Compute d parameter using equation 2
        const double d = -dot(N, first);

        // Compute t (equation 3)
        const double t = -(dot(N, r.origin()) + d) / NdotRayDirection;
        ...
        const vec3 O = r.origin() + r.direction();

        // Step 2: Inside-Outside Test
        // Test sidedness of P w.r.t. edge v0v1
        const vec3 v0p = P - first;
        ...
        const vec3 v2v1 = third - second;
        const vec3 v1p = P - second;
        Ne = cross(v2v1, v1p);
        if (dot(N, Ne) < 0.0) {
            return -1.0; // P is on the right side
        }
        ...
        const vec3 v2v0 = first - third;
        const vec3 v2p = P - third;
        Ne = cross(v2v0, v2p);
        if (dot(N, Ne) < 0.0) {
            return -1.0; // P is on the right side
        }
